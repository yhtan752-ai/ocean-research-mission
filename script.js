...
window.__finOnDone = null;      // callback waiting on whatever line Fin is saying right now
window.__finSkipAll = false;    // set true by the "Skip All" button; resets on next page load

// --------------------------------------------------------------------------
// FIN SKIP CONTROLS — "Skip" advances one line, "Skip All" fast-forwards
// every remaining line on the page. Injected once, shown only while Fin talks.
// --------------------------------------------------------------------------
let finSkipWrap = null;
function ensureFinSkipControls() {
    if (finSkipWrap) return finSkipWrap;
    finSkipWrap = document.createElement('div');
    finSkipWrap.id = 'fin-skip-controls';
    finSkipWrap.innerHTML =
        '<button type="button" id="fin-skip-btn" class="fin-skip-btn" title="Skip to the next line">Skip &#9654;</button>' +
        '<button type="button" id="fin-skip-all-btn" class="fin-skip-btn fin-skip-all-btn" title="Skip all of Fin\u2019s dialogue on this page">Skip All &#9654;&#9654;</button>';
    document.body.appendChild(finSkipWrap);

    finSkipWrap.querySelector('#fin-skip-btn').addEventListener('click', function () {
        finAdvance();
    });
    finSkipWrap.querySelector('#fin-skip-all-btn').addEventListener('click', function () {
        window.__finSkipAll = true;
        finAdvance();
    });
    return finSkipWrap;
}
function showFinSkipControls() { ensureFinSkipControls().classList.add('visible'); }
function hideFinSkipControls() { if (finSkipWrap) finSkipWrap.classList.remove('visible'); }

// Cuts off whatever Fin is currently saying and immediately runs the callback
// that was waiting on it — exactly as if the clip had simply finished.
function finAdvance() {
    const cb = window.__finOnDone;
    window.__finOnDone = null;
    finCancel();
    if (cb) cb();
}

// Waits `ms` before calling cb, unless "Skip All" is active, in which case it
// moves on immediately. Used for the little pauses between lines/phases.
function finDelay(ms, cb) {
    setTimeout(cb, window.__finSkipAll ? 0 : ms);
}
...
function finSpeak(audioFile, text, onDone) {
    finCancel();
    window.__finOnDone = onDone;
    showFinSkipControls();

    if (window.__finSkipAll) {
        // Fast-forward: skip the clip entirely and hand off to the next step.
        setTimeout(finAdvance, 0);
        return;
    }
    ...
    const finishAll = () => {
        window.__finSpeaking = false;
        if (window.__finOnDone === onDone) window.__finOnDone = null;
        if (onDone) onDone();
    };
    if (audioFile) {
    ...
    if (lineIndex < currentLines.length) {
        ...
        finSpeak(audioFile, text, function () {
            ...
            if (currentPhase === "intro" && lineIndex === currentLines.length) {
                ...
                // Dialogue done — hide the skip controls and show the continue button
                hideFinSkipControls();
    ...
    if (phaseFiveIndex < phaseFiveLines.length) {
        ...
        finSpeak(phaseFiveAudio[phaseFiveIndex], text, function () {
            ...
            if (phaseFiveIndex < phaseFiveLines.length) {
                ...
                // Lines finished -> hide skip controls, then play the AI video
                hideFinSkipControls();
                if (aiModelVideo) {
                    aiModelVideo.play()
                        .then(() => {
...
(function () {
    ...
    setTimeout(function () {
        finSpeak('ending.mp3', closingLine, hideFinSkipControls);
    }, 600);
